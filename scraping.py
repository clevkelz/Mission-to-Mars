...
def mars_hemispheres(browser):
    ...
    for x in range(4):
        ...
        # Find the image using the 'Sample' tag and extract the href for that piece.
        sample_elem = browser.links.find_by_text('Sample').first
        hemispheres['img_url'] = sample_elem['href']
        # Get the hemisphere titles.
        title = browser.find_by_css('h2.title').text
        hemispheres['title'] = title
        # Convert the titles and images to a list.
        hemisphere_image_urls.append(hemispheres)
        # Use the back feature to go back through the images for the four hemispheres until done.
        browser.back()
